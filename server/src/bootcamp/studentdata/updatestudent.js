...
export const UpdateStudent = async (data,res) => {
    try {
        const students = await db1.collection("Hackathondata").findOneAndUpdate({Reg_No:data?.reg},{$set:{Name:data?.name,Gmail:data?.gmail,Branch:data?.branch,Number:data?.number,Score:data?.score}})
