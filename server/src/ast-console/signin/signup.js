...
export const ConsoleRegister = async (data, res) => {
    ...
    try {
        ...
        if (admin) {
            ...
            await db1.collection('Hacthonadmin').insertOne({ Gmail: mail, Event: event, Number: phone, Password: password, Club: club, Date: date, NoOfAdmins: members, Admin: true,Admins:[mail] })
                .then((details) => {
