...
export const Admin=()=>{
    ...
    const Submit=async()=>{
        try{
            const responce=await axios.post("https://attendance-server1.vercel.app/admincheck/"+gmail+"/"+password)
            {
                if(responce.data)
                {
                 if (responce.data.Dates !== date.toDateString())
                 {
                    const res1=await axios.post("https://attendance-server1.vercel.app/totaldays")
                    {
                        if(res1.data)
                        {
                            if (res1.data.Date !== date.toDateString())
                            {
                                ...
                                const res = await axios.post("https://attendance-server1.vercel.app/updateadmin/"+gmail + "/" + date.toDateString() + "/" + tdays)
                                {
                                    if (res)
                                    {
...
export const Adminreg=()=>
{
    ...
    const Submit=async()=>
    {
        const res=await axios.post("https://attendance-server1.vercel.app/admincheck/"+gmail+"/"+password)
        {
            if (res.data) {
                alert("Already Register")
            }
            else {
                if (await axios.post("https://attendance-server1.vercel.app/adminregi/" + gmail + "/" + password)) {
